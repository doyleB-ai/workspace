---
subject: john-cleary
type: fact
last_verified: 2026-03-21
related: [facts/infrastructure, areas/nds-team]
---

# John Cleary

## Professional
- Full name: John Cleary
- Age: 36
- Role: Senior Manager at Amazon
- Team: Network Deployment Services (NDS)
- Location: Seattle, WA
- Reports to: Director of Network Engineering for Amazon Operations & Whole Foods Stores
- Team size: 59 FTEs + 42 contractors (India), levels L4-L6
- Team structure:
  1. Amazon operations & Whole Foods site launches
  2. Operational sites management
  3. ISP circuit procurement & operational support (global)

## Background
- Education: Degree in Geology
- Started at Amazon as a geologist contractor managing inventory systems
- AWS: 5 years
- Current role (NDS): Network provisioning, lifecycle management, ISP support for Amazon's global fulfillment, transportation, and grocery sites
- Non-traditional technical background — learned networking/operations on the job

## Career Goals
- Get Cisco certifications: CCNA, CCNP
- Transition into a network development role
- Increase compensation
- Reach director level at Amazon
- Open to moving companies if it accelerates career growth

## Working Style
- Tech-savvy but not a traditional sysadmin — learning as we go
- Direct communicator — gets to the point quickly
- Will say when something is too much work; find a more efficient path
- Learns by doing — walk through step by step with context
- **Cost-conscious:** Wants visibility into what services consume (tokens, API credits, model choices)
- **Verification-first:** Prefers to test and confirm (e.g., "run $2 worth of work to confirm auth is working") rather than assume

## Security Posture
- Security conscious — deliberately keeps clearyclaw as a non-admin account
- Admin account is separate (username: cleary on the Mac mini)
- Manages his own credentials — Doyle doesn't handle or store passwords

## Personal
- Family: wife + 2 young boys
- Lives in Seattle, WA
- Interests:
  - Poker (played less since having kids)
  - Snowmobiling & snowboarding (winter)
  - Seattle sports: Seahawks, Mariners
- Named Doyle after Doyle Brunson (poker legend) — calm, reads situations, doesn't waste words
